...
module.exports.execute = async (postcode) => {
  ...
  const result = await _getAddressFromSupplier(normalisedPostcode, buildRequest)
...
function _normalisePostcode (postcode) {
  ...
  if (!postcodeParts) return new Error('Postcode failed sanitisation')
  postcodeParts.shift()
...
async function _getAddressFromSupplier (normalisedPostcode, url) {
  try {
